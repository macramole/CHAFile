...
ADDRESSEE_TAG = "[+ %s]"
ADDRESSEE_ALT_TAG = "%%xds"
ADDRESSEE_ALT_TAG_TEMPLATE = "[%%%%xds: %s]"
ADDRESSEE_ALT_TAG_CHILD = "C"
ADDRESSEE_ALT_TAG_OTHER = "O"
ADDRESSEE_ALT_TAG_UNKNOWN = "U"
ADDRESSEE_ALT_TAG_ADULT = "A"
ADDRESSEE_ALT_TAG_TARGET_CHILD = "T"
#BOTH ?
...
USE_ALT_TARGET_CHILD = False #esto usara T en vez de C para el tag alternativo. ACLEW's ROS y CAS usa esto

ADDRESSEE_CORRESPOND = {
    ADDRESSEE_ALT_TAG_CHILD : SPEAKER_TARGET_CHILD,
    ADDRESSEE_ALT_TAG_OTHER : SPEAKER_OTHER,
    ADDRESSEE_ALT_TAG_UNKNOWN : SPEAKER_UNKNOWN,
    ADDRESSEE_ALT_TAG_ADULT : SPEAKER_ADULT,
    ADDRESSEE_ALT_TAG_TARGET_CHILD : SPEAKER_TARGET_CHILD,
}
ADDRESSEE_CORRESPOND_ALT = {
    ADDRESSEE_ALT_TAG_CHILD : SPEAKER_OTHER_CHILD,
    ADDRESSEE_ALT_TAG_OTHER : SPEAKER_OTHER,
    ADDRESSEE_ALT_TAG_UNKNOWN : SPEAKER_UNKNOWN,
    ADDRESSEE_ALT_TAG_ADULT : SPEAKER_ADULT,
    ADDRESSEE_ALT_TAG_TARGET_CHILD : SPEAKER_TARGET_CHILD,
...
#        exitcode, output = getstatusoutput(  os.path.join(CLAN_BIN_PATH, "kwal") + " +d4 -f " + strTiers + " \"" + self.chaFilePath + "\" " )
        ...
        for row in xmlOutput.getElementsByTagName("Row"):
            ...
            strLine = content[6:]
            addressee, strLine = self._getAddressee(strLine)
            toChild = addressee == SPEAKER_TARGET_CHILD #deprecate
            
            if self.CDS_ONLY and not toChild:
                continue

            for stopWord in STOP_WORDS:
                strLine = strLine.replace(stopWord, "").strip()

            line = {
                LINE_UTTERANCE : strLine,
                LINE_NUMBER : int(lineNumber),
                LINE_SPEAKER : speaker,
                LINE_TOCHILD : toChild, #deprecate
                LINE_ADDRESSEE : addressee
            }
            ...
            for tier in tiers:
                ...
                if hasattr(self, "_parse%s" % tierName.capitalize() ):
                    ...
                    line[tierName] = line[tierName][2:].split(":")

            self.lines.append(line)

    def _getAddressee(self, strLine):
    ...
    #DEPRECATED!
    def countUtterancesByTarget(self):
        childDirected = 0
        adultDirected = 0
        
        for line in self.lines:
            if line[LINE_TOCHILD]:
                childDirected += 1
            else:
                adultDirected += 1
        
        return (childDirected, adultDirected)
...
#a = ChaFile( "/home/leandro/Code/ciipme/tiposPalabras/data/paper/10-angelina-nsm-1audio.cha" )
#a = ChaFile( "/home/leandro/Data/QOM/Tiago/tiagogutiérrez1ro290315.cha", USE_TIERS = [ TIER_BULLET ] )
